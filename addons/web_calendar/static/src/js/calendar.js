...
// Dhtmlx scheduler ?
    init: function(parent, dataset, view_id, options) {
        ...
        this.dataset_events = [];
        this.COLOR_PALETTE = ['#f57900', '#B80000', '#4D9E3F', '#6F83F7', '#845B84','#AA0087', '#345E39', '#75507b', '#3465a4', '#6E9F3E', '#c17d11', '#D1C037', '#806767', '#ABC4B4', '#21763F', '#5E71B1', '#B284C3'];
		this.color_map = {};
        this.last_search = [];
...
/*        scheduler.attachEvent('onEmptyClick', function(start_date, mouse_event) {
        ...
        for (var e = 0; e < events.length; e++) {
            ...
            if (this.color_field) {
                ...
                if (filter) {
                    ...
                    if (typeof(fn_filter) === 'function' && !fn_filter(filter_value, evt)) {
                        continue;
                    }
                    var filter_item = {
                        value: filter_value,
    ...
    quick_save: function(event_id, event_obj) {
        ...
        if (index !== null) {
            ...
            this.dataset_events[index].date = data.date;
            this.dataset_events[index].date_deadline = data.date_deadline;
        }
    },
...
instance.web_calendar.SidebarFilter = instance.web.Widget.extend({
    ...
    filter_click: function(e) {
        ...
        if (responsibles.length) {
            this.view.events_loaded(this.view.dataset_events, function(filter_value, evt) {
                var user_ids = _.map(evt['user_ids'], function(n) { return n.toString(); } );
                var responsibles_in_user_ids = _.intersection(user_ids, responsibles);
                return _.indexOf(responsibles, filter_value.toString()) > -1 || !_.isEmpty(responsibles_in_user_ids);
            }, true);
        } else {
